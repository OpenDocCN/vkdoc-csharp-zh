...
2.  TrackScript(第[章第 9](https://doi.org/10.1007/978-1-4842-7851-2_09) :第一部分)

3.  PlayerState(第[章第 9](https://doi.org/10.1007/978-1-4842-7851-2_09) :第一部分)
...
下一个字段是`rearCamera`字段。这款相机被用作哈弗赛车的后视相机。接下来，我们有非常重要的`car`字段。该字段引用代表比赛中玩家的游戏对象。他们的悬停赛车。该组中的下一个也是最后一个字段是代表汽车前视传感器的`carSensor`游戏对象。我们已经在前面的高级交互课程复习第五章 T4 中讨论过这个问题。让我们转到下一组类字段。
