...
`FileStream`类以适合基于文件的流的方式为抽象`Stream`成员提供了一个实现。这是一条原始的河流；它只能读取或写入单个字节或字节数组。然而，你并不经常需要与`FileStream`类型的成员直接交互。相反，你可能会使用各种*流包装器*，这使得处理文本数据或.NETCore类型。尽管如此，您会发现尝试一下`FileStream`类型的同步读/写功能是很有帮助的。
