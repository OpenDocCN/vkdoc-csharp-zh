...
不算太寒酸！如果您调用这两个方法，您将看到相同的输出。然而，当使用`dynamic`关键字时，您为自己节省了相当多的工作。使用动态定义的数据，您不再需要手动将参数打包为对象数组、查询程序集元数据或其他类似的详细信息。如果您正在构建一个大量使用动态加载/后期绑定的应用程序，我相信您可以看到这些代码节省是如何随着时间的推移而增加的。
