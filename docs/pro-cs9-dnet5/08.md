...
如果下游接口想要提供自己的默认实现，它必须隐藏上游实现。例如，如果`IAdvancedDraw TimeToDraw()`方法需要 15 个单位来绘制，则将界面更新为以下定义:
...
但是，请注意，接口是。NET 核心框架。不管你开发的应用是什么类型(基于网络的，桌面图形用户界面，数据访问库，等等)。)，使用接口将是这个过程的一部分。总结一下到目前为止的情况，记住接口在以下情况下非常有用:
