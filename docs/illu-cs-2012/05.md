...
方法体是一个*块*，它(正如你在《??》第二章中回忆的那样)是一系列花括号之间的语句。一个块可以包含以下项目:
...
类型名后面的方括号空集指定参数将是一个`int` s 的*数组*，这里不需要担心数组的细节。他们在第十二章中有详细介绍。但是，对于我们这里的目的，您只需要知道以下内容:
...
例如，下面的代码声明了三个方法。方法`Main`调用`MethodA`，?? 调用`MethodB`，这样就创建了三个堆栈框架。当方法退出时，堆栈展开。
...
?? 图 5-17 说明了代码。注意，输入值为 3 时，方法`Count`有四个不同的独立堆栈帧。每个都有自己的输入参数值`inVal`。
