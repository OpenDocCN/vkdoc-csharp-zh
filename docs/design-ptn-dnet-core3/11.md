...
典型的操作系统终端的一个问题是，如果你通过管道向它输入大量数据，它会非常慢(T2)。比如一个 Windows 终端窗口(`cmd.exe`)使用 GDI 来渲染字符，完全没有必要。在快节奏的交易环境中，您希望渲染是硬件加速的:角色应该呈现为使用 API(如 OpenGL)放置在表面上的预渲染纹理。 <sup>3</sup>
