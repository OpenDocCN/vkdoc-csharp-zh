...
与传统的实体框架不同，在应用的第一次数据库访问期间，实体框架核心不检查模式是否是最新的。程序可能会在出现错误时运行(例如，“无效的列名‘邮政编码’”)。通过调用 context 类的`Database`对象中的方法`Migrate()`，您可以在启动时确保数据库模式是最新的(参见清单 [7-4](#Par90) )。`Migrate()`可能会执行缺失的迁移步骤，这是可能的，因为迁移类是项目编译的一部分，项目包含上下文类。
