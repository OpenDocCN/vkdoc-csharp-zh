...
本章将讨论*实验、概率分布、事件、互补事件、*和*互斥的概念。*这些概念将帮助读者理解密码和密码分析机制的基本概念，以及它们是如何根据概率设计的 [1 ]。
...
**定义 2.5** 。让我们考虑两个*互斥*事件，*E*1 和*E*2。如果下面的表达式等于 0，*p*(*e*<sub>1</sub>⋂*e*<sub>2</sub>)= 0，则它们是互斥的。一两个事件的出现将排除其他事件发生的可能性。

定义 2.6。让我们考虑以下两个事件， *E* <sub>1</sub> 和*E*2。
...
**定义 2.8** 。让我们考虑以下两个事件， *E* <sub>1</sub> 和*E*2。说他们是*独立*如果*p*(*e*1⋂*e*e<sub>2</sub>)=*p*(*e*1)*p*(*e*2)。
...
![$$ Var(X)=E\left({\left(X-\mu \right)}²\right) $$](img/493660_1_En_2_Chapter_TeX_Equf.png)
...
![$$ {P}_2\left(a,h\right)\to 1-\exp \left(-\frac{h\left(h-1\right)}{2a}+O\left(\frac{1}{\sqrt{a}}\right)\right)\approx 1-\mathit{\exp}\left(-\frac{h²}{2a}\right). $$](img/493660_1_En_2_Chapter_TeX_Equk.png)

提供的演示将解释为什么概率分布被称为*生日惊喜*或*生日悖论* *。*一个 23 人的房间里至少有 2 个人生日相同的概率是*P*2(365，23) ≈ 0.507，大得惊人。数量 *P* <sub>2</sub> (365， *h* )的增加与 *h* 的增加一样快。举个例子，*P*2(365，30) ≈ 0.706。
...
同样，让我们考虑随机变量 *Y* 和 *Z* ，它们将取一组有限的值 [1 ]。
...
**定义 2.19**  [1 ] [5 。我们来考虑 *X* 和 *Y* ，两个随机变量。将*联合熵*定义为
...
**定义 2.21** 。让我们考虑两个整数， *a* 和 *b* 。假设有一个整数 *c* 存在，那么 *a 除以 b* 使得*b*=*a*c。如果我们处于 *a* 除以 *b* 的情况，那么我们可以说 *a* ∣ *b* 。
...
我们将考虑两个非负整数， *a* 和 *b* ，每个小于 *n* 或等于 *n* 。请记住， *n* 的二进制表示中的位数是⌊ *lg n* ⌋ + 1，这个数字将由 lg *n* 来近似表示。表 2-1 总结了使用经典算法对整数进行四种基本运算所涉及的位操作数。
...
| *问* | *r* | *x* | *y* | *答* | *b* | *x*2 | *x*T2 1 | *y*2 | *y*T2 1 |
...
**定义 2.31** 。我们来考虑一下*m*∈*ℤ*<sub>*p*</sub>。 *m* 模 *p* 的*乘逆*的定义是一个整数*y*∈*ℤ*<sub>*p*</sub>这样 *m y* ≡ 1 ( *mod p* )。如果我们有这样的 *m* 存在，那么这个 *m* 是唯一的，并且 *m* 被称为*可逆*或*单元。*m*的倒数*记为*m*1。
...
**定义 2.36**  [1 。让我们把 *p* 看作一个奇数质数，把*看作一个整数。*勒让德符号*记为![$$ \left(\frac{a}{p}\right) $$](img/493660_1_En_2_Chapter_TeX_IEq10.png)，定义如下:*
...
**属性 2.37。勒让德符号的属性**[**1****。让我们考虑以下性质，称为勒让德符号的性质。我们认为 p 是一个奇素数。两个整数 *a* ， *b* ∈ ℤ.在这种情况下，将考虑 Legendre 符号的以下属性:**
...
2.  ![$$ \left(\frac{ab}{p}\right)=\left(\frac{a}{p}\right)\left(\frac{b}{p}\right). $$](img/493660_1_En_2_Chapter_TeX_IEq15.png)既然如果![$$ a\in {\mathbb{Z}}_p^{\ast } $$](img/493660_1_En_2_Chapter_TeX_IEq16.png)，那么![$$ \left(\frac{a²}{p}\right)=1\. $$](img/493660_1_En_2_Chapter_TeX_IEq17.png)

3.  如果*a*≦*b*(*mod p*)，那么![$$ \left(\frac{a}{p}\right)=\left(\frac{b}{p}\right). $$](img/493660_1_En_2_Chapter_TeX_IEq18.png)

4.  ![$$ \left(\frac{2}{p}\right)={\left(-1\right)}^{\frac{\left({p}²-1\right)}{8}}. $$](img/493660_1_En_2_Chapter_TeX_IEq19.png)自![$$ \left(\frac{2}{p}\right)=1 $$](img/493660_1_En_2_Chapter_TeX_IEq20.png) if * p * ≡ 1 或 7 ( * mod * 8)，if * p * ≡ 3 或 5 ( * mod * 8)。
...
**属性 2.39。属性为雅各比符号**[**1****。认为 *m* ≥ 3、 *n* ≥ 3 为奇数，认为 *a* 、 *b* ∈ ℤ.雅可比符号具有以下属性:**
...
2.  ![$$ \left(\frac{ab}{n}\right)=\left(\frac{a}{n}\right)\left(\frac{b}{n}\right) $$](img/493660_1_En_2_Chapter_TeX_IEq25.png)。因此如果![$$ a\in {\mathbb{Z}}_n^{\ast }, $$](img/493660_1_En_2_Chapter_TeX_IEq26.png)那么![$$ \left(\frac{a²}{n}\right)=1 $$](img/493660_1_En_2_Chapter_TeX_IEq27.png)。
...
7.  ![$$ \left(\frac{2}{n}\right)={\left(-1\right)}^{\frac{n²-1}{8}}. $$](img/493660_1_En_2_Chapter_TeX_IEq34.png)因此![$$ \left(\frac{2}{n}\right)=1 $$](img/493660_1_En_2_Chapter_TeX_IEq35.png) if * n * ≡ 1 或 7 ( * mod * 8)，if * n * ≡ 3 或 5 ( * mod * 8)。
...
7.  *如果一个* <sub>1</sub> = 1，*则返回*(*s*)；*else return*(*s*雅可比(*n*1，*a*1)。
...
**定义 2.43** 。假设一个*F*<sub>q</sub>表示一个有限阶域*q*=*p*<sup>*m*</sup>， *p* 是素数，那么![$$ {\mathbbm{F}}_q $$](img/493660_1_En_2_Chapter_TeX_IEq41.png)的特征是 *p* 。不止如此，![$$ {\mathbbm{F}}_q $$](img/493660_1_En_2_Chapter_TeX_IEq42.png)还会有一个ℤ <sub>* p *</sub> 的副本作为子字段，因为![$$ {\mathbbm{F}}_q $$](img/493660_1_En_2_Chapter_TeX_IEq43.png)可以看作是程度 *m* 的ℤ <sub>* p *</sub> 的扩展字段。

### 多项式和欧几里德算法

以下两个算法对于理解我们如何获得两个多项式的 *gcd* ， *g* ( *x* )和 *h* ( *x* )，两者都在ℤ<sub>*p*</sub>[*x*]是必要的:

**算法 2.43。** **欧几里得算法** **为ℤ**<sub>***p***</sub>***x*******1****
 ...
 ***算法 2.44。扩展的欧几里德算法为ℤ**<sub>***p***</sub>***x*******1****
...
下面的应用处理计算某一事件在一定数量的试验中发生的概率。下面的应用解释了这样一个事实，即一个事件在每次试验中都有发生的概率，在经过 *N* 次试验后，我们得到一次试验导致该事件发生的概率为 1(1*P*)<sup>N</sup>。见图 2-1 和清单 2-1 。
...
欧几里德算法有两个版本，*普通版*(见图 2-7 和清单 2-7 )和*扩展版*(见图 2-8 和清单 2-8 ) *。*两个版本的欧几里德算法的区别在于，常规版本基于连续的减法运算来计算两个数字之间的 GCD，而扩展版本应用于多项式(参见算法 2.43 和算法 2.44)。
